...
namespace Agi
{
	...
	Common::JSONArray inWorldOutput;
	const char* understood = "%m8"; // "I don't understand..." response.

	std::string inworld_open_session_url = "https://studio.inworld.ai/v1/workspaces/%s/characters/%s:openSession";
	std::string inworld_base64 = "cTgwcGlDdVV3TmNPS001bmdNcmFJQkRMVUZ3Y0VBeU06SmxMS3I2MGRLTHhwVm5sMXpsTzlid1NielBnMkdBUXBEejk0TXkySnQ0TVgycXpjVFBTSlJRUjBBT2RzS09hbA==";
	const char* inworld_name = "workspaces/%s/characters/%s";
	std::string inworld_sessionId = "";
	std::string inworld_characterId = "";
	std::string inworld_characterName = "ego";
	const char* inworld_workspaceId = "scummvmnext_13";
	Common::String commandText = "";

	void Agi::TextMgr::InWorldSessionResponse(Networking::JsonResponse response) {
		Common::JSONValue* json = response.value;
		inworld_sessionId = json->asObject().getVal("name")->asString().c_str();
		inworld_characterId = json->asObject().getVal("sessionCharacters")->asArray()[0]->asObject().getVal("character")->asString().c_str();
		debug(inworld_sessionId.c_str());
		debug(inworld_characterId.c_str());
	}

	void Agi::TextMgr::InWorldResponse(Networking::JsonResponse response) {
		Common::JSONValue* json = response.value;
		inWorldOutput = json->asObject().getVal("textList")->asArray();
	}
	...
	bool TextMgr::messageBox(const char* textPtr) {
		...
		{
			Networking::JsonCallback innerCallbackSession = new Common::Callback<TextMgr, Networking::JsonResponse>(this, &TextMgr::InWorldSessionResponse);
			Networking::CurlJsonRequest* request_session = new Networking::CurlJsonRequest(innerCallbackSession, nullptr, Common::String::format(inworld_open_session_url.c_str(), inworld_workspaceId, inworld_characterName.c_str()));
			...
			_workingRequest = ConnMan.addRequest(request_session);

			while (_workingRequest->state() == Networking::PROCESSING) {
				g_system->delayMillis(500);
			}

			Networking::JsonCallback innerCallback = new Common::Callback<TextMgr, Networking::JsonResponse>(this, &TextMgr::InWorldResponse);
			Networking::CurlJsonRequest* request = new Networking::CurlJsonRequest(innerCallback, nullptr, Common::String::format("https://studio.inworld.ai/v1/workspaces/%s/sessions/%s/sessionCharacters/%s:sendText", inworld_workspaceId, inworld_sessionId.c_str(), inworld_characterId.c_str()).c_str());

			request->addHeader(Common::String::format("Content-Type: %s", "application/json"));
			request->addHeader(Common::String::format("authorization: Basic %s", inworld_base64.c_str()));
			request->addHeader(Common::String::format("Grpc-Metadata-session-id: %s", inworld_sessionId.c_str()));

			const Common::JSONValue _jsonData = Common::String::format("{\"text\":\"%s\"}", (char*)_promptPrevious);
			request->addPostField(_jsonData.asString());
			...
			if (inWorldOutput.size() > 0) {
				for (int reply = 0; reply < inWorldOutput.size(); reply++) {
					textPtr = inWorldOutput[reply]->asString().c_str();
					debug(inWorldOutput[reply]->asString().c_str());
